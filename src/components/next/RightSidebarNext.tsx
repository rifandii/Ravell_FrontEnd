...
const RightSidebarNext = () => {
  const { tags, recentArticles: latestArticles, loading } = useGlobalData();
  const pathname = usePathname();
  const { headings } = useSidebar();
  const isArticleDetailPage = pathname.startsWith('/articles/') && headings.length > 0;
      ...
      <ul className="space-y-4">
        {latestArticles.map((article) => (
          <li key={article.id} className="group">
            <Link href={`/articles/${article.slug}`} className="block cursor-pointer">
...
export default RightSidebarNext;
